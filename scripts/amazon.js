...
let addButtonTimeOutId;

document.querySelectorAll('.js-add-to-cart').forEach((button) => {
  button.addEventListener('click', () => {
    const { productId } =  button.dataset; //note that the name product-name gets converted to productName (kebab case to camel case)
    ...
    addedToCart.classList.add('added-button');
    


    clearTimeout(addButtonTimeOutId);
    
    addButtonTimeOutId =  setTimeout(() => {
      addedToCart.classList.remove('added-button');
    }, 2000);
    ...
    cart.forEach((item) => {
      if (productId === item.productId)
        matchingItem = item; // copia il rieferimento, NON clona un oggetto nuovo
    });
    if (matchingItem) {
